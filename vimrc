...
filetype off " изночально было on
filetype plugin on
filetype indent on
...
" больше не поддерживается Bundle 'Valloric/YouCompleteMe'
" репозитории vim/scripts
Bundle 'L9'
Bundle 'FuzzyFinder'
Bundle 'rails.vim'

" репозитории не на github
...
vmap <F7> <esc>:bn<cr>i
imap <F7> <esc>:bn<cr>i
" F9 - сохранение файла и вызов bash
map <F9> :w<cr>:!/bin/bash<cr>
vmap <F9> <esc>:w<cr>:!/bin/bash<cr>i
imap <F9> <esc>:w<cr>:!/bin/bash<cr>i
map <A-F9> :w<cr>:make<cr>
vmap <A-F9> <esc>:w<cr>:make<cr>i
imap <A-F9> <esc>:w<cr>:make<cr>i
...
"let g:airline#enablexi#syntastic#enable = 1
"let g:airline#enable#bufferline = 1

" Подцветка синтаксиса для Arduino-фойлов (ino,pde)
